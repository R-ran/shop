...
export const getStaticPaths: GetStaticPaths<ParsedQueryParams> = async ({
  ...
  try {
    ...
    console.warn('Failed to fetch types during build, using fallback mode:', error);
    return {
...
> = async ({ locale, params }) => {
  ...
  try {
    ...
    // 如果 API 调用失败，返回基本 props，让页面可以渲染
    console.warn('Failed to fetch data during build, using fallback:', error);
    return {
      props: {
        variables: {
          popularProducts: { type_slug: 'book', limit: 10, with: 'type;author', language: locale },
          products: { type: 'book', limit: PRODUCTS_PER_PAGE },
          categories: { type: 'book', limit: CATEGORIES_PER_PAGE, language: locale, parent: 'null' },
          bestSellingProducts: { type_slug: 'book', limit: 10, with: 'type;author', language: locale },
          layoutSettings: {},
          types: { type: 'book' },
        },
        layout: 'default',
